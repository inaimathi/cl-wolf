...
(defmacro container ((&rest label/part-pairs) &body connections)
  (multiple-value-bind (ins outs) (find-container-ports connections)
    `(let ((self (make-instance 'container))
	   ,@(mapcar (lambda (pair)
		       (cond ((and (symbolp pair) (module-exists? pair))
			      `(,pair (,pair)))
			     ((symbolp pair)
			      (error "Can't find module ~s. CONTAINER clauses must be valid modules." pair))
			     ((or (eq 'reactor (car (second pair)))
				  (module-exists? (car (second pair))))
			      `(,(first pair) ,(second pair)))
			     (t
			      (error "~s is neither a REACTOR literal nor a call to a MODULE." pair))))
		     label/part-pairs))
